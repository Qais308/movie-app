...
export default function Header({ isOpen, setIsOpen }) {
    ...
    const timeout = setTimeout(async () => {
      try {
        ...
        const omdbRes = await fetch(
          `https://www.omdbapi.com/?apikey=${omdbApiKey}&s=${encodeURIComponent(
            search
          )}&type=series`
        );
        const omdbData = await omdbRes.json();
        const series =
          omdbData.Search?.filter(
  ...
  className="flex-1 ml-6 sm:max-w-[250px] md:w-[200px] xl:w-[400px] relative"
>
          ...
          <input
            ...
            className="w-full px-2 py-1 rounded bg-gray-800 text-white focus:outline-none ml-2"
          />
        ...
        {/* ✅ Small Logo (directly, no extra box) */}
        <MovieLogo size="sm" />
      </div>
    </header>
  );
