...
export default function HeroSection() {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [loaded, setLoaded] = useState(false); // NEW: wait for first 2 images
  const navigate = useNavigate();

  useEffect(() => {
    // Preload all images as before
    preloadImages(featuredMovies.map((m) => m.poster));

    // Preload the first 2 images before rendering
    const firstTwo = featuredMovies.slice(0, 2).map((m) => {
    ...
    Promise.all(firstTwo).then(() => setLoaded(true));
  ...
  // Don't render HeroSection until first 2 images are loaded
