...
// const highlightStat = (val1, val2) => (val1 > val2 ? "text-green-400" : val1 < val2 ? "text-red-400" : "");
...
//   const handleSelectMovie = (movie, isFirst) => {
//     if (isFirst) {
//       setFirstMovie(movie);
//       setFirstQuery(movie.title);
//       fetchOMDb(movie.imdb_id, setFirstOMDb);
//       setFirstSuggestions([]);
//     } else {
//       setSecondMovie(movie);
//       setSecondQuery(movie.title);
//       fetchOMDb(movie.imdb_id, setSecondOMDb);
//       setSecondSuggestions([]);
//     }
...
//       <h1 className="text-3xl font-bold mb-4 text-center">Compare Movies</h1>
//       <p className="text-center mb-6 text-gray-300">
//         Search and select two movies to compare their details side by side.
//       </p>

//       {/* Search Section */}
//       <div className="flex flex-col md:flex-row gap-4 md:gap-6">
//         <div className="flex-1 relative" ref={firstRef}>
//           <input
//             type="text"
...
//                   )}
//                   <span>{m.title} ({m.release_date?.slice(0, 4) || "N/A"})</span>
//                 </li>
//               ))}
//             </ul>
//           )}
...
//         <div className="flex-1 relative" ref={secondRef}>
//           <input
//             type="text"
...
//               fetchSuggestions(e.target.value, setSecondSuggestions);
//             }}
//             className="w-full p-2 rounded bg-gray-800 text-white focus:outline-none"
//           />
//           {secondSuggestions.length > 0 && (
//             <ul className="absolute z-10 w-full bg-gray-900 rounded mt-1 shadow-lg">
//               {secondSuggestions.map((m) => (
//                 <li
//                   key={m.id}
//                   className="flex items-center gap-3 p-2 cursor-pointer hover:bg-gray-700 transition-all duration-150"
//                   onClick={() => handleSelectMovie(m, false)}
//                 >
//                   {m.poster_path ? (
//                     <img
//                       src={`https://image.tmdb.org/t/p/w92${m.poster_path}`}
...
//                 <p className={highlightStat(movie.runtime, otherMovie.runtime)}>
//                   <strong>Runtime:</strong> {movie.runtime || "N/A"} min
//                 </p>
//                 <p className={highlightStat(movie.vote_count, otherMovie.vote_count)}>
//                   <strong>Vote Count:</strong> {movie.vote_count || "N/A"}
//                 </p>
//                 <p className={highlightStat(movie.popularity, otherMovie.popularity)}>
...
const TMDB_KEY = "46b71fe47d81e124380aeddcf9b37ccd";
const OMDB_KEY = "ad50766f";
const TRAKT_CLIENT_ID = "d69bd3f14de6c67337b84b2e0769391b15a26e6110897e978c416136b77d0345"; // replace with your client ID

const highlightStat = (val1, val2) =>
  val1 > val2 ? "text-green-400" : val1 < val2 ? "text-red-400" : "";
...
const ComparePage = () => {
  ...
  const fetchSuggestions = async (query, setSuggestions) => {
    ...
    try {
      ...
      console.error("TMDB suggestion fetch error:", err);
    }
  };
  ...
  // --- Fetch Trakt stats ---
  const fetchTraktDetails = async (title, year, type) => {
    try {
      const res = await fetch(
        `https://api.trakt.tv/search/${type}?query=${encodeURIComponent(
          title
        )}&year=${year}`,
        {
          headers: {
      ...
      const slug = results[0][type === "movie" ? "movie" : "show"].ids.slug;
      const statsRes = await fetch(
        `https://api.trakt.tv/${type}s/${slug}/stats`,
        {
          headers: {
            "Content-Type": "application/json",
            "trakt-api-version": "2",
      ...
      return { ...results[0][type === "movie" ? "movie" : "show"], stats };
    } catch (err) {
  ...
  const handleSelectMovie = async (item, isFirst) => {
    ...
    const traktDetails = await fetchTraktDetails(
      item.title || item.name,
      item.release_date?.slice(0, 4) || item.first_air_date?.slice(0, 4),
      type
    );
    if (isFirst) {
      setFirstMovie({ ...tmdbDetails, trakt: traktDetails });
      setFirstQuery(item.title || item.name);
      fetchOMDb(tmdbDetails.imdb_id, setFirstOMDb);
      setFirstSuggestions([]);
    } else {
      setSecondMovie({ ...tmdbDetails, trakt: traktDetails });
    ...
    <div className="max-w-6xl mx-auto p-4 text-white">
      ...
      <div className="flex flex-col md:flex-row gap-4 md:gap-6">
        {[{ query: firstQuery, setQuery: setFirstQuery, suggestions: firstSuggestions, setSuggestions: setFirstSuggestions, ref: firstRef, isFirst: true },
          { query: secondQuery, setQuery: setSecondQuery, suggestions: secondSuggestions, setSuggestions: setSecondSuggestions, ref: secondRef, isFirst: false }].map((s, idx) => (
          ...
          {[firstMovie, secondMovie].map((movie, idx) => {
            ...
            return (
              ...
              >
                ...
                {movie.trakt?.stats && otherMovie.trakt?.stats && (
                  <>
                    <p className={highlightStat(movie.trakt.stats.watchers, otherMovie.trakt.stats.watchers)}>
                      <strong>Trakt Watchers:</strong> {movie.trakt.stats.watchers || 0}
                    </p>
                    <p className={highlightStat(movie.trakt.stats.plays, otherMovie.trakt.stats.plays)}>
                      <strong>Plays:</strong> {movie.trakt.stats.plays || 0}
                    </p>
                    <p className={highlightStat(movie.trakt.stats.collected, otherMovie.trakt.stats.collected)}>
                      <strong>Collected:</strong> {movie.trakt.stats.collected || 0}
                    </p>
                  </>
                )}
                {omdb && otherOMDb && (
