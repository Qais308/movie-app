...
const PersonDetails = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const creditsRes = await fetch(`https://api.themoviedb.org/3/person/${id}/movie_credits?api_key=${TMDB_KEY}`);
      const creditsData = await creditsRes.json();
      // Only include movies with poster_path
      setCredits((creditsData.cast || []).filter(movie => movie.poster_path));
    }
    fetchData();
  }, [id]);
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 text-white">
      ...
      <div className="mb-6">
        ...
        <div className="flex gap-3 sm:gap-4 md:gap-4 overflow-x-auto pb-2">
          {topMovies.map(movie => (
            <div key={movie.id} className="flex-shrink-0 w-28 sm:w-32 md:w-36">
              <img
                src={`https://image.tmdb.org/t/p/w200${movie.poster_path}`}
                alt={movie.title}
                className="rounded-lg shadow-md w-full h-44 sm:h-48 md:h-52 object-cover"
              />
              <p className="text-xs sm:text-sm md:text-base mt-1 text-center truncate">
